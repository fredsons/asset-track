...
const fazerLogin = async () => {
  ...
  try {
    const res = await axios.post(
      "https://asset-track-api.onrender.com/login",
      loginForm.value,
    );
...
const verHistorico = async (ativo) => {
  try {
    const res = await axios.get(
      `https://asset-track-api.onrender.com/ativos/${ativo.id}/historico`,
    );
...
const gerenciarAtivo = async (ativo) => {
  ...
  if (ativo.funcionarioId) {
    ...
    if (res.isConfirmed) {
      await axios.patch(
        `https://asset-track-api.onrender.com/ativos/${ativo.id}/devolver`,
      );
      buscarDados();
  ...
  if (fid) {
    await axios.patch(
      `https://asset-track-api.onrender.com/ativos/${ativo.id}/atribuir`,
...
const salvarAtivo = async () => {
  if (idAtivoEmEdicao.value)
    await axios.put(
      `https://asset-track-api.onrender.com/ativos/${idAtivoEmEdicao.value}`,
      formAtivo.value,
    );
  else
    await axios.post(
      "https://asset-track-api.onrender.com/ativos",
      formAtivo.value,
    );
  cancelarEdicaoAtivo();
  buscarDados();
...
const salvarFuncionario = async () => {
  if (idFuncionarioEmEdicao.value)
    await axios.put(
      `https://asset-track-api.onrender.com/funcionarios/${idFuncionarioEmEdicao.value}`,
      formFuncionario.value,
    );
  else
    await axios.post(
      "https://asset-track-api.onrender.com/funcionarios",
      formFuncionario.value,
    );
...
const excluirAtivo = (id) =>
  confirmarExclusao(async () => {
    await axios.delete(`https://asset-track-api.onrender.com/ativos/${id}`);
    buscarDados();
  });
const editarFunc = (f) => {
...
const excluirFunc = (id) =>
  confirmarExclusao(async () => {
    await axios.delete(
      `https://asset-track-api.onrender.com/funcionarios/${id}`,
    );
    buscarDados();
  });
...
<template>
  <div class="min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      <div class="bg-white p-8 rounded-lg shadow-2xl w-full max-w-sm">
        ...
        <form @submit.prevent="fazerLogin" class="space-y-4">
          <input
            ...
            type="email"
            placeholder="Email"
            class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:border-blue-500"
            required
          />
          <input
            ...
            required
          />
          <div v-if="loginErro" class="text-red-500 text-sm text-center">
            {{ loginErro }}
          </div>
          <button
            type="submit"
    ...
    <div v-else>
      <nav
        class="bg-slate-900 text-white h-16 flex items-center justify-between px-6 shadow-md sticky top-0 z-30"
      >
        ...
        <div class="flex items-center gap-4 text-sm">
          ...
          <button
            @click="fazerLogout"
            class="bg-slate-700 hover:bg-red-600 px-3 py-1 rounded transition"
          >
            Sair
          </button>
        </div>
      ...
      <div class="max-w-7xl mx-auto px-4 py-8">
        <div class="flex border-b border-gray-300 mb-6 space-x-6">
          <button
            v-for="aba in ['dashboard', 'ativos', 'funcionarios']"
            :key="aba"
            @click="abaAtual = aba"
            class="pb-3 px-2 border-b-2 capitalize font-medium transition-colors"
            :class="
              abaAtual === aba
        ...
        <div v-if="abaAtual === 'dashboard'">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div
              class="bg-white p-5 rounded border border-gray-200 shadow-sm border-l-4 border-l-slate-500"
            >
              <div class="text-xs font-bold text-gray-400 uppercase">Total</div>
              <div class="mt-1 text-3xl font-bold">{{ stats.total }}</div>
            </div>
            <div
              class="bg-white p-5 rounded border border-gray-200 shadow-sm border-l-4 border-l-blue-500"
            >
            ...
            <div
              class="bg-white p-5 rounded border border-gray-200 shadow-sm border-l-4 border-l-emerald-500"
            >
              <div class="text-xs font-bold text-gray-400 uppercase">
                Disponíveis
              </div>
            ...
            <div
              class="bg-white p-5 rounded border border-gray-200 shadow-sm border-l-4 border-l-amber-500"
            >
              <div class="text-xs font-bold text-gray-400 uppercase">Valor</div>
              <div class="mt-1 text-3xl font-bold text-gray-700">
                {{
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div
              class="bg-white p-6 rounded shadow-sm border border-gray-200 h-[450px]"
            >
        ...
        <div v-if="abaAtual === 'ativos'">
          <div
            class="bg-white p-6 rounded border border-gray-200 shadow-sm mb-6"
          >
            <h2
              class="text-sm font-bold text-gray-800 uppercase mb-4 pb-2 border-b"
            >
              {{ idAtivoEmEdicao ? "Editar" : "Novo" }} Registro
            </h2>
            <form
              @submit.prevent="salvarAtivo"
            ...
            >
              ...
              <div class="md:col-span-2">
                ...
                ><input
                  id="input-serial"
                  v-model="formAtivo.serialNumber"
                  class="w-full border border-gray-300 rounded p-2 text-sm"
                  required
                />
              </div>
              ...
              <div class="md:col-span-1">
                <button
                  v-if="idAtivoEmEdicao"
                  type="button"
                  @click="cancelarEdicaoAtivo"
                  class="w-full mb-1 border py-1 text-xs"
                >
                  ✕</button
                ><button
                  type="submit"
                  class="w-full bg-blue-700 text-white py-2 rounded text-sm font-bold hover:bg-blue-800"
                >
                  Salvar
                </button>
              </div>
          ...
          <div class="flex justify-between items-center mb-4">
            <input
              v-model="termoBusca"
              class="border border-gray-300 rounded p-2 text-sm w-64"
              placeholder="Buscar..."
            /><span class="text-xs text-gray-500"
              >{{ ativosPaginados.length }} itens</span
            >
          </div>

          <div
            class="bg-white border border-gray-200 rounded shadow-sm overflow-hidden"
          >
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-200">
                <tr
                  v-for="a in ativosPaginados"
                  :key="a.id"
                  class="hover:bg-gray-50"
                >
                  <td class="px-6 py-3">
                    <div class="font-medium text-sm">{{ a.nome }}</div>
                    <div class="text-xs text-gray-500">{{ a.tipo }}</div>
                  </td>
                  <td class="px-6 py-3">
                    ...
                    <div class="text-xs text-gray-400">
                      {{
                        (a.preco || 0).toLocaleString("pt-BR", {
                          style: "currency",
                          currency: "BRL",
                        })
                      }}
                  ...
                  <td class="px-6 py-3 text-right flex justify-end gap-1">
                    ...
                    <button
                      ...
                      "
                    >
                      {{ a.funcionarioId ? "Devolver" : "Atribuir" }}
                    </button>
                    <span class="text-gray-300 mx-1">|</span>
                    <button
                      @click="clonarAtivo(a)"
                      class="text-emerald-600 p-1"
                    >
                      📄
                    </button>
                    <button @click="editarAtivo(a)" class="text-blue-600 p-1">
                      ✏️
                    </button>
                    <button
                      @click="excluirAtivo(a.id)"
            ...
            <div
              class="px-4 py-3 border-t flex justify-between bg-gray-50"
              v-if="totalPaginas > 1"
            >
              <button
                @click="paginaAtual--"
        ...
        <div v-if="abaAtual === 'funcionarios'">
          ...
          >
            <h2
              class="text-sm font-bold text-gray-800 uppercase mb-4 pb-2 border-b"
            >
              {{ idFuncionarioEmEdicao ? "Editar" : "Novo" }}
            </h2>
            <form
            ...
            >
              ...
              <div class="md:col-span-3">
                <label
                  ...
                  >Setor</label
                ><select
                  v-model="formFuncionario.departamento"
                  class="w-full border border-gray-300 rounded p-2 text-sm bg-white"
                >
                  <option>TI</option>
                  <option>RH</option>
              ...
              <div class="md:col-span-12 flex justify-end gap-2 mt-2">
                ...
                ><button
                  type="submit"
                  class="bg-blue-700 text-white px-4 py-2 rounded text-sm font-bold shadow-sm"
                >
                  Salvar
                </button>
              </div>
          ...
          >
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                >
                  <td class="px-6 py-4">
                    <div class="font-medium text-sm">{{ f.nome }}</div>
                  ...
                  <td class="px-6 py-4 text-right">
                    <button @click="editarFunc(f)" class="text-blue-600 mr-2">
                      ✏️</button
                    ><button @click="excluirFunc(f.id)" class="text-red-600">
                      🗑️
                    </button>
                  </td>
                </tr>
