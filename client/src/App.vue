...
// URL API
const API_URL = "https://asset-track-api.onrender.com";
...
const abrirPerfil = () => {
  formPerfil.value = {
    novoNome: usuarioLogado.value.nome,
    novoEmail: usuarioLogado.value.email,
    novaSenha: '',
    senhaConfirmacao: ''
  };
  exibindoPerfil.value = true;
};
...
const totalPaginas = computed(() =>
  Math.ceil(ativosFiltrados.value.length / itensPorPagina),
);
...
const stats = computed(() => {
  ...
  const disponivel = ativos.value.filter(
    (a) => a.status === "Disponível",
  ).length;
  const valorTotal = ativos.value.reduce(
  ...
  datasets: [
    {
      ...
      data: [
        stats.value.disponivel,
        stats.value.emUso,
        ativos.value.filter((a) => a.status === "Manutenção").length,
      ],
    },
  ],
}));
...
const chartDataDept = computed(() => {
  ...
  funcionarios.value.forEach((f) => {
    if (f.ativos?.length)
      deptCount[f.departamento] =
        (deptCount[f.departamento] || 0) + f.ativos.length;
  });
  return {
...
const verHistorico = async (ativo) => {
  try {
    const res = await axios.get(
      `${API_URL}/ativos/${ativo.id}/historico`,
    );
    if (res.data.length === 0) {
      Toast.fire({ icon: "info", title: "Sem histórico" });
      return;
    }
...
const gerenciarAtivo = async (ativo) => {
  if (ativo.funcionarioId) {
    ...
    if (res.isConfirmed) {
      await axios.patch(`${API_URL}/ativos/${ativo.id}/devolver`);
      buscarDados();
  ...
  if (!funcionarios.value.length) {
    Swal.fire("Aviso", "Cadastre colaboradores primeiro.", "warning");
    return;
  }

  const options = funcionarios.value
    .map(
      (f) => `<option value="${f.id}">${f.nome} (${f.departamento})</option>`,
    )
    .join("");
  ...
  if (fid) {
    await axios.patch(
      `${API_URL}/ativos/${ativo.id}/atribuir`,
      { funcionarioId: Number(fid) },
    );
    buscarDados();
    Toast.fire({ icon: "success", title: "Atribuído!" });
  }
};
...
const salvarAtivo = async () => {
  if (idAtivoEmEdicao.value)
    await axios.put(`${API_URL}/ativos/${idAtivoEmEdicao.value}`, formAtivo.value);
  else
    await axios.post(`${API_URL}/ativos`, formAtivo.value);
  cancelarEdicaoAtivo();
  buscarDados();
...
const salvarFuncionario = async () => {
  if (idFuncionarioEmEdicao.value)
    await axios.put(`${API_URL}/funcionarios/${idFuncionarioEmEdicao.value}`, formFuncionario.value);
  else
    await axios.post(`${API_URL}/funcionarios`, formFuncionario.value);
  formFuncionario.value = { nome: "", email: "", departamento: "TI" };
  idFuncionarioEmEdicao.value = null;
  buscarDados();
  Toast.fire({ icon: "success", title: "Salvo!" });
};

const clonarAtivo = (a) => {
  formAtivo.value = { nome: a.nome, tipo: a.tipo, serialNumber: "", preco: a.preco };
  idAtivoEmEdicao.value = null;
  window.scrollTo({ top: 0, behavior: "smooth" });
  setTimeout(() => document.getElementById("input-serial").focus(), 500);
};

const editarAtivo = (a) => {
  formAtivo.value = { ...a };
  idAtivoEmEdicao.value = a.id;
  window.scrollTo({ top: 0, behavior: "smooth" });
};

const cancelarEdicaoAtivo = () => {
  formAtivo.value = { nome: "", tipo: "Notebook", serialNumber: "", preco: 0 };
  idAtivoEmEdicao.value = null;
};

const excluirAtivo = (id) =>
  confirmarExclusao(async () => {
    await axios.delete(`${API_URL}/ativos/${id}`);
    buscarDados();
  });

const editarFunc = (f) => {
  formFuncionario.value = { ...f };
  idFuncionarioEmEdicao.value = f.id;
  window.scrollTo({ top: 0, behavior: "smooth" });
};
    ...
    <div v-else>
      <nav class="bg-slate-900 text-white h-16 flex items-center justify-between px-6 shadow-md sticky top-0 z-30">
        ...
        <div class="flex items-center gap-4 text-sm">
          <span class="font-medium">{{ usuarioLogado.nome }}</span>
          <button @click="abrirPerfil" class="text-slate-300 hover:text-white transition flex items-center gap-1">
            <span>⚙️ Perfil</span>
          </button>
      ...
      <div v-if="exibindoPerfil" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
          <div class="bg-slate-800 text-white p-4 flex justify-between items-center">
            <h3 class="font-bold uppercase text-sm tracking-widest">Minha Conta</h3>
            <button @click="exibindoPerfil = false" class="text-slate-400 hover:text-white">✕</button>
          </div>
          <form @submit.prevent="atualizarPerfil" class="p-6 space-y-4">
            <div>
              <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Nome Completo</label>
              <input v-model="formPerfil.novoNome" type="text" class="w-full border border-gray-300 p-2 rounded text-sm focus:border-blue-500 outline-none" required />
            </div>
            <div>
              <label class="block text-xs font-bold text-gray-500 uppercase mb-1">E-mail de Acesso</label>
              <input v-model="formPerfil.novoEmail" type="email" class="w-full border border-gray-300 p-2 rounded text-sm focus:border-blue-500 outline-none" required />
            </div>
            <div class="pt-2 border-t">
              <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Nova Senha (opcional)</label>
              <input v-model="formPerfil.novaSenha" type="password" class="w-full border border-gray-300 p-2 rounded text-sm focus:border-blue-500 outline-none" placeholder="Deixe em branco para manter" />
            </div>
            <div>
              <label class="block text-xs font-bold text-blue-600 uppercase mb-1">Senha Atual (Para salvar)</label>
              <input v-model="formPerfil.senhaConfirmacao" type="password" class="w-full border-2 border-blue-100 p-2 rounded text-sm focus:border-blue-500 outline-none bg-blue-50" required placeholder="Digite sua senha para confirmar" />
            </div>
            <div class="flex gap-2 pt-4">
              <button type="button" @click="exibindoPerfil = false" class="flex-1 border py-2 rounded text-sm font-bold hover:bg-gray-50">Cancelar</button>
              <button type="submit" class="flex-1 bg-blue-700 text-white py-2 rounded text-sm font-bold hover:bg-blue-800">Salvar Alterações</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div v-if="abaAtual === 'dashboard'">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white p-6 rounded shadow-sm border border-gray-200 h-[450px]"><h3 class="text-xs font-bold text-gray-400 uppercase mb-4">Status</h3><Doughnut :data="chartDataStatus" :options="chartOptions" /></div>
            <div class="bg-white p-6 rounded shadow-sm border border-gray-200 h-[450px]"><h3 class="text-xs font-bold text-gray-400 uppercase mb-4">Departamento</h3><Bar :data="chartDataDept" :options="chartOptions" /></div>
          </div>
        </div>
        ...
        <div v-if="abaAtual === 'funcionarios'">
          ...
          <div class="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr><th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Nome</th><th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Email</th><th class="px-6 py-3 text-right">Ações</th></tr></thead>
              <tbody class="divide-y divide-gray-200">
                <tr v-for="f in funcionarios" :key="f.id" class="hover:bg-gray-50">
                  <td class="px-6 py-4"><div class="font-medium text-sm">{{ f.nome }}</div><div class="text-xs text-gray-500">{{ f.departamento }}</div></td>
                  <td class="px-6 py-4 text-sm text-gray-600">{{ f.email }}</td>
                  <td class="px-6 py-4 text-right"><button @click="editarFunc(f)" class="text-blue-600 mr-2">✏️</button><button @click="excluirFunc(f.id)" class="text-red-600">🗑️</button></td>
                </tr>
              </tbody>
            </table>
          </div>
